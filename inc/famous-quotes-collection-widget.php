...
class Famous_Quotes_Collection_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if( $instance ) {
			$options['title'] = isset($instance['title'])?$instance['title']:__('Random Famous Quote', 'famous-quotes-collection');
	...
	public function form( $instance ) {
		...
		if( $instance ) {
			...
			if( isset( $instance['char_limit'] ) && is_numeric( $instance['char_limit'] ) ) {
				...
				$options['char_limit'] = __('none', 'famous-quotes-collection');
			}
			$options['tags'] = $instance['tags'];
		}
		else {
		...
		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'title' ).'">'.__( 'Title', 'famous-quotes-collection' ).'</label>';
		...
		echo '<label for="'.$this->get_field_id( 'show_author' ).'">'.__( 'Show author?', 'famous-quotes-collection' ).'</label>';
		echo '</p>';


		echo '<p>';
		echo '<input type="checkbox" id="'.$this->get_field_id( 'ajax_refresh' ).'" name="'.$this->get_field_name('ajax_refresh').'"'.$ajax_refresh_checked.' />';
		echo '<label for="'.$this->get_field_id( 'ajax_refresh' ).'">'.__( 'Refresh feature', 'famous-quotes-collection' ).'</label>';
		echo '</p>';

		echo "<p style=\"text-align:left;\"><small><a id=\"quotescollection-adv-key\" style=\"cursor:pointer;\" onclick=\"jQuery('div#quotescollection-adv-opts').slideToggle();\">".__('Advanced options', 'famous-quotes-collection')." &raquo;</a></small></p>";
		...
		echo '<label for="'.$this->get_field_id( 'random_refresh' ).'">'.__( 'Random refresh', 'famous-quotes-collection' ).'</label>';
		echo '<br/><span class="setting-description"><small>'.__('Unchecking this will rotate quotes in the order added, latest first.', 'famous-quotes-collection').'</small></span></p>';
		echo '</p>';

		echo '<p>';
		echo '<input type="checkbox" id="'.$this->get_field_id( 'auto_refresh' ).'" name="'.$this->get_field_name('auto_refresh').'"'.$auto_refresh_checked.' />';
		echo '<label for="'.$this->get_field_id( 'auto_refresh' ).'">'.__( 'Auto refresh', 'famous-quotes-collection' ).'</label>';
		echo ' <label for="'.$this->get_field_id( 'refresh_interval' ).'"">';
		printf( __('every %s sec', 'famous-quotes-collection'), '<input type="number" id="'.$this->get_field_id( 'refresh_interval' ).'" name="'.$this->get_field_name('refresh_interval').'" value="'. esc_attr( $options['refresh_interval'] ).'" min="3" max="60" step="1" style="width:3em;" />' );
		echo '</label>';
		echo '</p>';
		
		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'tags' ).'">'.__( 'Tags filter', 'famous-quotes-collection' ).'</label>';
		echo '<input class="widefat" id="'.$this->get_field_id( 'tags' ).'" name="'.$this->get_field_name( 'tags' ).'" type="text" value="'.esc_attr( $options['tags'] ).'">';
		echo '<br/><span class="setting-description"><small>'.__('Comma separated', 'famous-quotes-collection').'</small>';
		echo '</p>';

		echo '<p>';
		echo '<label for="'.$this->get_field_id( 'char_limit' ).'">'.__( 'Character limit', 'famous-quotes-collection' ).'</label>';
	...
	public function update( $new_instance, $old_instance ) {
		...
		if(!$instance['char_limit'] || !is_numeric($instance['char_limit']))
			$instance['char_limit'] = __('none', 'famous-quotes-collection');
	...
	private function default_options() {
		return array(
			'title'               => __('Random Famous Quote', 'famous-quotes-collection'), 
			'show_author'         => 1,
			'ajax_refresh'        => 1,
			'auto_refresh'        => 0,
			'random_refresh'      => 1,
